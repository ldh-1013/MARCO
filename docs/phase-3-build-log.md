...
> ⚠ **이 스프린트가 GAP-22의 다른 절반을 깨뜨렸다**(2/3 Net 재검증에서 발견). 스프린트 13이 GAP-22에 함께 적었던 **"새 플레이어가 들어와도 기존 술래가 바뀌지 않는다"**는 술래가 항상 정렬 0번이라는 전제에서 나온 성질이다. 로테이션이 순번을 `roundNumber % playerCount`로 옮기면서 **순번이 인원수에 의존**하게 돼, 라운드 중 한 명만 합류해도 술래가 교체되거나(새 순번이 다른 사람을 가리킴) 아예 사라졌다(새 순번이 태그당한 플레이어를 가리켜 `IsTaggedOut` 스킵에 걸림).
>
> 이제 그 보장은 **정렬 규칙이 아니라 순번 고정**에서 나온다 — `RoundNetworkSync._fixedSeekerOrder`가 라운드 시작 시점 값을 얼려 두고, 라운드 중 합류자는 러너로만 채운다. 상세와 테스트는 `phase-4-build-log.md`의 "전체 재검증 (2/3)" 절 참고.
...
이번에는 절 번호가 정확했다(§12.6 "설정 화면 상세 스펙"). 도식은 `[오디오] [비디오] [조작] [게임플레이]` 탭 + `[기본값 복원] [닫기]`이며 항목은 다음과 같다:

| 탭 | 항목 | 기본값 |
|---|---|---|
| 오디오 | 마이크 장치 선택 / 입력 게인(-20~+20dB) / 발화 감도 재보정 / 마스터 볼륨 / 효과음·UI 볼륨(개별) | 시스템 기본 / 0dB / — / 80% / 80% |
| 비디오 | FOV(75°~100°) / 밝기(파문 대비) / 색맹 모드 / 프레임 표시 | 90° / 중간 / 끔 / 끔 |
| 조작 | 키 리바인딩(§4.3 전체) / 마우스 감도 / Y축 반전 | — |
| 게임플레이 | 발화 방식(VAD/PTT) / 파문 유형 자막 / 오프닝 가이드 재시청 | VAD / 끔 / — |

진입점은 §12.1 타이틀 화면 우하단의 "설정 ⚙"이다(§12.6 자체에는 진입점 명시 없음).

### ⚠ 15항목 중 9항목은 선행 시스템이 없다 (GAP-42)

코드에서 데이터 소스를 전수 확인한 결과:

- **마이크 장치·입력 게인·발화 감도 재보정·VAD/PTT** → §5.2 음성 파이프라인 미구현.
- **효과음/UI 개별 볼륨** → 프로젝트에 `AudioMixer`가 없다(`AudioListener`만 존재).
- **키 리바인딩** → §4.3 표 전체 재배정 시스템 없음(각 컴포넌트가 `Key` 필드를 개별 보유).
- **파문 유형 자막·오프닝 가이드 재시청** → 해당 기능 자체가 없음.
- **밝기(파문 대비)** → 무엇을 조절하는지 정의가 없다.

이 항목들은 **값조차 만들지 않았다**. 만들어 두면 "설정은 있는데 아무 효과가 없는" 상태가 되어 실기에서 버그로 오인된다. 대신 화면 하단에 "준비 중(선행 시스템 필요)"으로 그대로 나열해, 스펙에서 빠뜨린 것이 아니라 **의존 기능 대기 중**임을 드러낸다(§12.2 "솔로 연습장 — 준비 중" 선례).

**구현한 6항목**: 색맹 모드, 마우스 감도, Y축 반전, FOV, 마스터 볼륨, 기본값 복원(+프레임 표시는 값만 저장).

### 구현

- **신규 Core `GameSettings`**: 기본값·유효 범위·손상값 복구(`Clamped`)만 갖는 순수 구조체. 기획서에 명시된 값만 상수로 못박았다(FOV 75~100/기본 90, 마스터 80%, 색맹·프레임 표시 끔). 마우스 감도는 명시가 없어(GAP-43) **기존 코드값 0.12를 기본으로 유지** — 설정 화면이 생겼다는 이유만으로 조작감이 달라지면 안 되기 때문이다.
- **신규 Presentation `SettingsStore`**: `PlayerPrefs` 입출력 + 적용. 규칙은 Core가 갖고 여기서는 Unity 객체만 만진다. 플레이어는 네트워크 스폰이라 **설정 변경 시점과 플레이어 준비 시점 양쪽**에서 적용한다(`LocalPlayerRegistry.WhenReady`).
- **신규 UI `SettingsScreen`**: 스프린트 16~18 패턴 그대로(런타임 uGUI·legacy Text·팔레트 폴백). F1 열기/Esc 닫기, ↑↓ 항목·←→ 값·Enter 실행. §12.6 도식의 탭·드롭다운·슬라이더 드래그는 포인터 UI 배선이 없어 키보드로 대체했다(GAP-44).
- **`FirstPersonController`**: `ApplyLookSettings(감도, Y반전)` 추가 + `_invertY` 필드. 반전은 기본 끔이라 **기존 조작이 그대로**다. 이동·태그 규칙은 건드리지 않았다.
- **색맹 모드는 기존 C 키와 같은 상태를 공유**한다(`PulseVisualRenderer.SetColorblindMode`) — 두 경로가 어긋나지 않는다. 단 C 키 토글은 저장되지 않는다(설정 화면 경유만 저장).
- **씬 배선**: `SceneFlowSetupTool`이 SceneFlow 오브젝트에 부착(2단계 생성 + 5단계 보강), 진단 도구가 미부착 시 ✖. `BootFlow`가 세션 시작에 저장값을 불러와 적용한다.

### 검증

- 신규 **9케이스**: 기본값이 §12.6과 일치, FOV 범위가 원문과 일치, 감도 기본값이 기존 조작감 유지, 상·하한 클램프, **NaN·무한대 복구**(손상된 PlayerPrefs 방어), 유효값 무변경, 기본값이 범위 안, 감도 범위 성립.
- 총계 412 → **421 전수 통과**. 런타임·에디터(경계)·합본 컴파일 각 0 error, 0 warning.
- **실기 검증 미실시** — 절차는 `수동검증_절차.md` §24(특히 §24-3 재시작 후 유지).

### 남은 작업

1. §5.2 음성 파이프라인 — 오디오 탭 4항목 + 최다 비명상(GAP-37)이 여기에 묶여 있다.
2. §3.2 메아리 노크 — 최고의 거짓말상 전제.
3. 키 리바인딩 시스템(§4.3) — 설정 조작 탭 완성 조건.
4. §10.1 "술래 격리 공간 3초 후 진입" 미구현.
5. GAP-19~44 정리.

---

## 스프린트 24 — 술래 격리 진입 (§10.1)

### §10.1 원문 — 표의 한 셀이 전부

> | 입구 로비 | 8×6m | 도망자 스폰 지점(**술래는 격리 공간에서 3초 후 별도 진입**) | 표준 | 라커룸 A/B, 메인 풀 홀 |

기획서 전체를 "격리"로 검색하면 이 한 곳뿐이다(나머지 1건은 §15 NetworkBridge의 무관한 "격리층"). 여기서 확정적으로 도출되는 것은 **대상(술래)**과 **대기 시간(3초)** 둘뿐이며, 나머지는 전부 미명시다:

| 미명시 항목 | 처리 |
|---|---|
| 격리 공간의 위치·크기 | **구역 구성표에 "격리 공간" 행 자체가 없다** → GAP-45, 앵커 오브젝트로 조정 가능하게 |
| 카운트다운 표시 여부 | GAP-46 → 미구현(Console 로그만) |
| "별도 진입"의 방식 | GAP-47 → 이동 잠금 해제로 구현(문 개방 등 연출 없음) |
| 3초의 기산점 | GAP-48 → §15.4 RoleAssign의 3초와 **별개**로 해석 |

**3초 기산점 해석의 근거**: §15.4 RoleAssign의 3초 카운트다운(역할 추첨 연출)은 **맵 로드 전 로비에서** 흐른다. 그 3초를 격리 시간으로 보면 "격리 공간에서 3초 후 진입"이라는 문장이 성립하지 않는다(그 시점에 맵도 격리 공간도 없다). 따라서 맵 배치 뒤에 흐르는 별개의 3초로 구현했다.

### 구현

- **신규 Core `SeekerIsolation`**: 대상 판정(`AppliesTo` — 술래만)과 3초 카운트다운. 리매치로 역할이 바뀌면 `Begin`이 이전 상태를 덮어쓴다(§2.3 로테이션과 맞물림).
- **신규 Core `IsolationAnchorRegistry`**: `SpawnAnchorRegistry`와 같은 좌표 스냅샷 패턴(맵이 애디티브라 Transform 참조는 위험).
- **신규 Presentation `SeekerIsolationAnchor`**: 맵 쪽 표식. 오브젝트를 옮기는 것만으로 위치를 조정할 수 있다(GAP-45 대응).
- **`PawnPhaseTeleporter` 분기**: 술래 → 격리 앵커(스폰 링 분산 없이 그 지점 그대로 — §6.2상 술래는 1인), 러너·메아리 → 종전 스폰 링. **앵커가 없어도 3초 대기는 건다** — §10.1의 3초는 위치가 아니라 시간 규칙이고, 앵커 누락으로 규칙까지 사라지면 안 되기 때문이다(경고 로그로 알린다).
- **`SceneFlowSetupTool` 3단계**가 앵커를 `(12, 0.05, 17)`에 생성 — 도망자 스폰에서 z −10m로, 스폰 링 지름(8m)보다 멀어 시작 즉시 조우하지 않으며 맵 바닥 범위(x 0~45, z 0~35) 안이다. `Setup Everything`에 이미 포함된 단계라 별도 배선이 필요 없다.
- **진단 도구**: 앵커 미존재 시 ✖.

### ⚠ 이동 잠금에 `ILocalControlGate`를 재사용하지 않은 이유

지시서 §1-5가 기존 패턴 재사용을 제안했으나, `SetLocalControl(false)`는 **소유권용**이라 입력뿐 아니라 카메라·커서를 끄고 **`LocalPlayerRegistry` 등록까지 해제**한다. 격리에 그것을 쓰면 로컬 플레이어 참조가 사라져 HUD·스폰 배치가 전부 어긋난다 — 스프린트 22에서 실제로 겪은 "등록 유실"과 같은 유형의 사고가 된다.

그래서 **이동만 막는 별도 게이트**(`FirstPersonController.SetMovementLocked`)를 추가했다. 시점 회전은 열어 둔다 — 눈까지 막으라는 규칙은 없고, 주변을 볼 수 있어야 "격리 공간에서 대기"가 성립한다.

### 검증

- 신규 **12케이스**: 3초 상수가 원문과 일치, 술래만 대상(러너·메아리 제외), 시작 즉시 잠김, 러너 즉시 자유, 3초 후 해제, 남은 시간 음수 방지, 0·음수 deltaTime 방어, **리매치 재시작 시 다시 잠김**, **로테이션으로 러너가 되면 이전 격리 해제**, Clear, 초기 상태.
- 총계 421 → **433 전수 통과**. 런타임·에디터(경계)·합본 컴파일 각 0 error, 0 warning.
- **실기 검증 미실시** — 절차는 `수동검증_절차.md` §25(특히 §25-3 이동만 막히고 시점은 되는지, §25-4 리매치 연동).

### 남은 작업

1. §5.2 음성 파이프라인 — 오디오 설정 4항목 + 최다 비명상(GAP-37)이 묶여 있다.
2. §3.2 메아리 노크 — 최고의 거짓말상 전제.
3. 키 리바인딩 시스템(§4.3) — 설정 조작 탭 완성 조건.
4. GAP-19~48 정리.
...
## 스프린트 25 — GAP 전수 정리 + Phase 3 종료 판정

코드 변경 없음(문서 정리 전용). GAP-1~48을 **코드 파일 실재 여부까지 확인**해 분류했다 — 문서 기록만으로 "해소됨"으로 올리지 않았다.

### GAP 집계

| 상태 | 건수 | 대표 |
|---|---|---|
| ✅ 해소 | **19** | GAP-22(호스트 항상 술래 → 로테이션), GAP-16~18(서버 권위), GAP-32(낙하 복구) |
| 📌 결정 유지 | **17** | 스펙 공백을 결정으로 메운 것 — 플레이테스트 조정 대상 |
| ⏭ Phase 4 이월 | **11** | GAP-37/42가 대표 — §5.2 음성·§3.2 노크·§4.3 키 리바인딩 선행 |
| 🧊 Deprecated | **1** | GAP-23(로비 게이트가 생기며 "즉시 배정"이 폐기됨) |

상세 표는 `프로젝트_현황_종합.md` §4-A.

### 코드 실재 검증 결과

해소를 주장한 항목의 산출물이 실제로 존재하는지 파일 단위로 확인했다 — `TagRules` · `ServerRoundDriver` · `RoleAssigner` · `SeekerRotation` · `SpawnRing` · `FallRecovery` · `SeekerIsolation` · `AwardTally` · `GameSettings` · `ServerLobbyDriver` · `RematchVoteDriver` · `SceneFlowController` · `SettingsScreen` · `SeekerIsolationAnchor` **전부 존재**. `Assets/_Project/DebugTools/` **부재 확인**(스프린트 19 제거 완료).

### Phase 3 완료 체크리스트 — **4항목 중 3항목 충족**

| # | 기준 | 결과 |
|---|---|---|
| 1 | 모든 핵심 게임플레이 루프 **구현** | ✅ 밸브·태그·탈출·라운드·역할·발소리·로비·스폰·낙하복구·술래로테이션·어워드·설정·격리 전부 구현, 433 테스트 통과, 전 어셈블리 0 error/0 warning |
| 2 | 모든 핵심 루프 **실기 검증** | ❌ **미충족** — 아래 참조 |
| 3 | 네트워크 2단계 전체 실기 검증 | ✅ 2026-07-26 완료(밸브·태그·라운드·역할·파문 반경 소수점 일치) |
| 4 | `NetworkTestBootstrap`/DebugTools 제거 | ✅ 스프린트 19 완료(폴더 부재 확인) |

### ❌ 2번이 미충족인 이유 — 실기 검증 절 6개가 미완료

`수동검증_절차.md`의 다음 절이 아직 통과 기록이 없다:

| 절 | 대상 | 스프린트 |
|---|---|---|
| §19 | 로비 — 준비·게이팅·리매치 투표 | 18 |
| §20 | 씬 분리 — MainMenu·맵 지연 로드 | 18b |
| §21 | 낙하 복구 | 20 |
| §22 | 술래 로테이션 | 21 |
| §23 | 어워드 + 리매치 스폰 리셋 | 22 |
| §24 | 설정 화면(특히 재시작 후 저장 유지) | 23 |
| §25 | 술래 격리 진입 | 24 |

스프린트 18b 기간에 사용자 실기로 **렌더링·스폰·접속 버그 4건이 실제로 발견·수정**됐다(중복 카메라, 로비 임시 바닥 Z-파이팅/무한 낙하, 로컬 플레이어 등록 유실, 접속 컴포넌트 비활성화). 이는 **구현 완료와 실기 통과가 다르다는 것을 이 프로젝트에서 직접 증명한 사례**다. 같은 이유로 남은 6개 절을 통과 없이 완료로 기록하지 않는다.

또한 **스프린트 17의 `NetworkObject.OnDestroy` NRE 재검토**(§20-7)가 여전히 미결이다 — 맵 언로드로 밸브 3개가 동시 파괴되는 새 시나리오가 스프린트 18b에서 생겼고, Play 도중 재현 여부가 확인되지 않았다.

### 판정: **Phase 3 코드 완료(code-complete)** — 정식 종료는 실기 검증 통과 후

- **선언한 것**: Phase 3의 모든 계획 기능이 구현·테스트·문서화됐다. 신규 기능 작업은 종료한다.
- **선언하지 않은 것**: "Phase 3 완료". 체크리스트 2번이 충족되지 않았다.
- **종료 조건**: §19~§25 실기 검증 통과 + §20-7 NRE 판정. 통과하면 그 시점에 Phase 3 완료를 기록하고 Phase 4로 넘어간다.

이 판정은 지시서 §4 "GAP을 임의로 해소됨으로 분류하지 말 것"과 같은 원칙을 완료 선언 자체에 적용한 것이다.

### Phase 4 이월 항목 (11건, 근거 절 포함)

| 이월 항목 | GAP | 기획서 근거 |
|---|---|---|
| §5.2 음성 분류 파이프라인 | 4, 6, 37, 42 | §5.2 전체 + §20.1 MVP 포함 목록 |
| §3.2 메아리 노크 | 37, 39 | §3.1 특수 능력 "노크(30초 쿨다운, 3.2절)" |
| §4.3 키 리바인딩 | 42, 44 | §12.6 조작 탭 "키 리바인딩 \| 4.3절 표 전체 재배정" |
| 방코드 매칭(Steam) | 28 | §13 멀티플레이 세션 설계 |
| 솔로 연습장 | 31 | §12.2 메인 메뉴 세 번째 버튼 |
| 플레이어 이름 체계 | 41 | §12.3 로비 도식의 닉네임 |
| 세트/판수 표시 | 36 | §2.3 "3판 1세트" |
| 격리 카운트다운 표시 | 46 | §10.1 |


---

## ✅ Phase 3 완료 (2026-08-04)

스프린트 25에서 "코드 완료(code-complete), 정식 종료는 실기 검증 통과 후"로 유보했던 판정을 **완료로 확정**한다.

### 유보를 푼 근거

| 조건 | 상태 |
|---|---|
| `수동검증_절차.md` §19~§25 실기 통과 | ✅ **2026-07-29 전 항목 통과**(사용자 실기) |
| 스프린트 17 `NetworkObject.OnDestroy` NRE 재검토(§20-7) | ✅ 관찰 완료 — 이월 사유 소멸 |
| 클라이언트 밸브 미표시(스프린트 25 후속) | ✅ 해소 — 아래 참조 |

§19~§25는 로비 게이팅·씬 분리·맵 지연 로드·낙하 복구·술래 로테이션·어워드·리매치 스폰 리셋·설정 저장·술래 격리까지 **스프린트 18~24의 전 산출물**을 덮는다. 이 구간이 통과함으로써 완료 체크리스트 2번(핵심 루프 실기 검증)이 충족됐다.

### 마지막 결함 — 클라이언트에서만 밸브가 보이지 않던 버그

Phase 3 마감 직전 2-클라이언트 실기에서 발견됐고, **클라이언트 `Player.log`로 원인을 확정**했다:
...
세 SceneId는 밸브 3개의 것이었고, 로그 순서상 **맵 로드 완료 직전에** 도착했다. 맵이 애디티브로 로드되는 §15.1 구조에서 서버는 자기 씬 로드 직후 씬 NetworkObject를 스폰하는데(`ServerObjects.SetupSceneObjects` — 벤더 주석의 전제는 "clients are not yet in the scene"), 관측 조건이 없으면 **아직 그 씬을 로드하지 않은 클라이언트에게도 즉시 전송**된다. 클라이언트는 그 SceneId를 모르니 스폰을 버리고 다시 받지 못한다. 호스트가 멀쩡했던 이유는 서버 겸임이라 클라이언트 쪽 비활성화 경로(`ClientObjects.RegisterAndDespawnSceneObjects`)를 건너뛰기 때문이다.

**수정**: 신규 `SceneObserverSetupTool`이 밸브에 `NetworkObserver` + `SceneCondition`을 부착한다(`Setup Everything` 9단계). `SceneCondition`은 `connection.Scenes.Contains(오브젝트의 씬)`일 때만 관측자로 인정하므로, 클라이언트가 로드를 마치고 `AddConnectionToScene`이 실행된 **뒤에** 스폰이 나간다. 전역 `ObserverManager` 기본 조건을 쓰지 않은 이유는 플레이어 pawn에도 적용되어 로비↔맵 전환 순간 서로 안 보일 위험이 있기 때문이다 — 문제가 확인된 맵 씬 오브젝트에만 붙이는 것이 최소 수정이다.

이 건은 **씬 분리(§15.1)가 만들어낸 구조적 결함**이었다. 단일 씬 시절에는 존재할 수 없던 경쟁이며, 애디티브 맵 로드를 도입한 대가를 마지막에 치른 셈이다.
...
| 스프린트 | 1~25 |
| 테스트 | **433 케이스 전수 통과** |
| 컴파일 | 런타임·에디터(경계 분리)·합본 **각 0 error / 0 warning** |
| 어셈블리 | Core / Net / Presentation / Editor (DebugTools 제거 완료) |
| GAP | 48건 전수 분류 — 해소 19 · 결정 유지 17 · Phase 4 이월 11 · Deprecated 1 |
| 실기 절차서 | §10~§25 (부록 포함) |

### 구현된 핵심 루프 (13)

밸브 상호작용 · 태그 · 탈출 · 라운드 판정 · 역할 자동 배정 · 발소리 파문(서버 권위 §5.6 차폐) · 로비 준비·게이팅 · 리매치 투표 · 씬 분리 + 맵 지연 로드 · 스폰 분산 · 낙하 복구 · 술래 로테이션 · 어워드 집계 · 설정 저장 · 술래 격리 진입

### 이 프로젝트에서 배운 것 (Phase 4로 가져갈 원칙)

1. **코드 완료 ≠ 실기 통과.** 스프린트 18b 기간에만 구현이 끝난 기능에서 버그 4건이 나왔다(중복 카메라 / 로비 바닥 Z-파이팅·무한 낙하 / 로컬 플레이어 등록 유실 / 접속 컴포넌트 비활성화). 전부 테스트·컴파일을 통과한 상태였다.
2. **기억이 아니라 파일을 열 것.** GUID를 기억으로 단정해 "빌트인 Standard 셰이더"라고 오진한 사례가 있었다(실제로는 URP/Lit). 벤더 소스·씬 YAML·로그를 직접 읽었을 때만 원인이 맞았다.
3. **판별 테스트를 먼저 설계할 것.** "Scene 뷰에서 눈높이로 봐도 어두운가"처럼 가설을 반증할 수 있는 5초짜리 확인이 여러 번 방향을 바로잡았다.
4. **경계 분리 빌드가 필수.** 합본 컴파일은 asmdef 경계(internal) 위반을 못 잡는다 — 런타임/에디터를 나눠 빌드해야 실제 어셈블리 구성과 같아진다.
5. **기획서 절 번호를 신뢰하지 말고 검색할 것.** 지시서가 지목한 §2.2·§6.5는 각각 §2.3·§8이었고, §6.5는 아예 존재하지 않았다.

**→ Phase 4 진행은 `phase-4-build-log.md`에 이어서 기록한다.**
