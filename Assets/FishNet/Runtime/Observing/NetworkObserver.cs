        ...
        internal void Deinitialize(bool destroyed)
        {
            ...
            if (_conditionsInitializedPreviously)
            {
                ...
                foreach (ObserverCondition item in _observerConditions)
                {
                    ...
                    /* [PATCHED, docs/phase-2-foundation.md 참조] Unity 6.5+ deprecated the
                     * int-based GetInstanceID() sign check in favor of EntityId, which has
                     * no direct negative-value equivalent (see Unity's InstanceID-to-EntityId
                     * migration guide). There is no drop-in replacement for "was this
                     * instantiated at runtime vs a reference to the original asset", so this
                     * cleanup is conservatively skipped on 6.5+ rather than risk destroying a
                     * shared/persistent ObserverCondition asset. Revisit once FishNet ships an
                     * official fix; this project does not use ObserverCondition yet. */
#else
